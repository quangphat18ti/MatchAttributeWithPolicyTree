...
func (c *Condition) findConditionType() (ConditionType, error) {
	if c.isOnlyField() {return ConditionType(FieldOnly), nil;}
	if c.isNumberRange() {return ConditionType(NumberRange), nil;}
	if c.isDateRange() {return ConditionType(DateRange), nil;}
	if c.isComparation() {return ConditionType(Comparation), nil;}
...
func (c *Condition) isNumberRange() bool {
	...
	if len(strings.Split(splits[0], " ")) != 1 {return false;}

	// right is seperate 2 pattern with -
	if len(strings.Split(splits[1], "-")) != 2 {return false;}
